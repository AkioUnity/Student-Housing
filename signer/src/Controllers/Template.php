...
class Template{
    ...
    public function dropboximport() {
        ...
        $data = array(
        				...
        				"source" => "url",
        				"document_key" => Str::random(32),
        				"activity" => 'Template Imported from Dropbox by <span class="text-primary">'.$actionTakenBy.'</span>.'
        			);
        $upload = Signer::upload($data);
        if ($upload['status'] == "success") {
	        exit(json_encode(responder("success", "", "","documentsCallback()", false)));
    ...
    public function googledriveimport() {
        ...
        $data = array(
        				...
        				"activity" => 'Template Imported from Google Drive by <span class="text-primary">'.$actionTakenBy.'</span>.'
        			);
        $upload = Signer::upload($data);
        header('Content-type: application/json');
        if ($upload['status'] == "success") {
